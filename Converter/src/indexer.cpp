...
void doIndexing(string targetDir, State& state, Options& options, Sampler& sampler) {
	...
	TaskPool<Task> pool(options.threadNumber, [&writeAndUnload, &state, &options, &activeThreads, tStart, &lastReport, &totalPoints, totalBytes, &pointsProcessed, chunks, &indexer, &nodes, &mtx_nodes, &sampler](auto task) {
